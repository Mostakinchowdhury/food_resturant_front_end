...
const HeadSection = async ({ product }: { product: product_type }) => {
  try {
    const avgRating = await axios.get<{ average_rating: number; review_count: number }>(
        ...
        <div className="w-full md:w-1/2 flex flex-col gap-4 flex-1">
          ...
          <div className="flex items-center gap-4">
            <p className="text-gray-600 text-size4 font-semibold">
              ...
              {product.max_price && parseFloat(product.max_price) > parseFloat(product.price) && (
